...
class Pixle(Attack):
    ...
    def restart_forward(self, images, labels):
        ...
        if self.transfer == False:
          # for each image id i'll have a list of 4 number that are the coordinates and other list that are the pixel mapping
          pixel_to_swap_per_image = {}
          for idx in range(bs):
              pixel_to_swap_per_image[bs] = {'coordinates' : [],
                                             'mappings' : []}
              ...
              for r in range(self.restarts):
                  ...
                  for it in range(self.max_patches):
                      ...
                      if p < best_p:
                          pixel_to_swap_per_image[bs]['coordinates'] += [solution[:4]]
                          pixel_to_swap_per_image[bs]['mappings'] += [solution[4:]]
                          best_p = p
              ...
              l_0.append(torch.norm((image - best_image), 0))
          ...
          for idx in self.pixel_to_swap_per_image_transfer.keys(): # for each img
            for element in range(len(self.pixel_to_swap_per_image_transfer[idx]['coordinates'])): # coordinates and mappings

              solution = self.pixel_to_swap_per_image_transfer[idx]['coordinates'][element] + self.pixel_to_swap_per_image_transfer[idx]['mappings'][element]
              pert_image = self._perturb(source=image,
                                               destination=best_image,
                                               solution=solution)
        ...
        if self.return_solutions:
            return adv_images, pixel_to_swap_per_image
