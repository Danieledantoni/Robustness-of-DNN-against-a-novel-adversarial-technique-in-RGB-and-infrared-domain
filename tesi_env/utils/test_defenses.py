...
def test_defense(cfg: test_att):
    ...
    if cfg.defense_configuration.name == 'detection':

        if cfg.defense_configuration.filter == 'amf':
            model_with_filter_det = Filter_detection(amf, cfg.defense_configuration.init_size, cfg.defense_configuration.max_size,
            cfg.defense_configuration.threshold, model)  
            setup = adversarial_test(model_with_filter_det, attack, test_loader)
        
        if cfg.defense_configuration.filter == 'median':
            model_with_filter_det = Filter_detection(ndimage.median_filter, cfg.defense_configuration.init_size, cfg.defense_configuration.max_size,
            cfg.defense_configuration.threshold, model)  
            setup = adversarial_test(model_with_filter_det, attack, test_loader)

    if cfg.defense_configuration.name == 'clean gan':

        gen = Large_Generator(3).to(device)
        opt_gen = optim.Adam(gen.parameters(), lr = 1e-3)
        load_generator(gen, opt_gen, cfg.defense_configuration.gan_path)
        model_with_gan = Clean_GAN(gen, model)
        setup = adversarial_test(model_with_gan, attack, test_loader)

    if cfg.defense_configuration.name == 'mitigation':

        model_with_rand_layers = Mitigation_rand(cfg.defense_configuration.start_mitigation, cfg.defense_configuration.stop_mitigation, 
        cfg.defense_configuration.vit_mitigation, model)
        setup = adversarial_test(model_with_rand_layers, attack, test_loader)
